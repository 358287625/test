...
function del(id){
	if(confirm("你确定删除吗？")){
		var url ="${basePath}user/del.do?m=${m}";
		var data={"d":id};
			...
			<div class="row">
				<div class="col-lg-12">
					<div class="panel panel-default">
						<div class="row">
							<form action="${basePath}user/list.do?m=${m}" method="post" id="doSearchFromId">
								<div class="col-lg-2">
									<div class="input-group" style="padding-left: 15px;">
										<input type="text" name="keyWord" value="${keyWord}" placeholder="请输入搜索关键字"
							...
							<div class="col-lg-8 col-lg-2">
								<div class="input-group pull-right">
									<span class="input-group-btn"> <!-- <button class="btn btn-sm btn-default">新增客户</button> -->
										<button type="button"
											onclick="go('view/jsp/user/addUser.jsp?m=${m}');"
											class="btn btn-info"
											style="float: right; margin-right: 18px;">新增用户</button>
									</span>
								</div>
							</div>
						</div>
							...
							<table id="datatable1" class="table table-striped table-hover">
								...
								<tbody>
									<c:forEach items="${pager.datas}" var="data" varStatus="s">
										...
										<td>${data.name}</td>
										<td><c:if test="${data.sex==0}">男</c:if><c:if test="${data.sex!=0}">女</c:if> </td>
										<td>${data.phone}</td>
										<td>${data.identityCard}</td>
										<td>
											<button type="button" onclick="javascript:location.href='${basePath}user/get.do?m=${m}&id=${data.id}'" class="mb-sm btn btn-success">修改</button>
